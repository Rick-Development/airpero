...
class SupportTicketController extends GetxController {
  ...
  Future getTicketList(
    ...
    if (response.statusCode == 200) {
      ...
      if (data['status'] == 'success') {
        final fetchedData = data['message']['tickets'];
        if (fetchedData.isNotEmpty) {
          ticketList.addAll(t.TicketListModel.fromJson(data).message!.tickets!);
          if (isLoadMoreRunning == false) {
            _isLoading = false;
          }
          if (kDebugMode) {
  ...
  Future getTicketView(
    ...
    if (response.statusCode == 200) {
      ...
      if (data['status'] == 'success') {
        viewTicketList
            .add(viewTicket.ViewTicketModel.fromJson(data).message!.tickets!);
        update();
      } else {
        ApiStatus.checkStatus(data['status'], data['message']);
      }
